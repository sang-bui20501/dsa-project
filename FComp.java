...
public class FComp extends JPanel{
	...
	public void paint(final Graphics g) {
		...
		final Rectangle background = Main.main.background;
		final Rectangle dirt = Main.main.dirt;
		...
		for (int j = 0; j < Main.main.pipe.size(); ++j) {
			final Pipe p = Main.main.pipe.get(j);
		...
		for (int k = 0; k < Main.main.bird.size(); ++k) {
			final Pair<Rectangle, Integer> bi = Main.main.bird.get(k);
			if (!Main.main.idx.contains(k)) {
				final Rectangle b = (Rectangle)bi.getKey();
