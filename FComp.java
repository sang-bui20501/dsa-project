...
import me.Goldensang.gamelogic.NeuralNetManager;
...
public class FComp extends JPanel {
	...
	public void paint(final Graphics g) {
		...
		for (int k = 0; k < GameState.getBirds().size(); ++k) {
			Bird b = GameState.getBirds().get(k);
			if (b.isAlive()) {
				Rectangle birdRec = b.getBird();
				g.drawImage(bird, birdRec.x, birdRec.y, birdRec.width, birdRec.height, null);
			}
		}
		g.setColor(Color.white);
		g.setFont(new Font("Arial", 1, 100));
		g.setFont(new Font("Arial", 1, 30));
		g.drawString("Score " + String.valueOf(GameState.getBestScore()), 50, 50);
		g.drawString("Gen:  " + String.valueOf(GameState.getGeneration()), 50, 100);

		NeuralNetwork bestBird = NeuralNetManager.getInstance().getBestBird();
